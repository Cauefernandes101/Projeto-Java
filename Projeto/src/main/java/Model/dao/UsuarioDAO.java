...
public class UsuarioDAO {
    ...
    /*public ResultSet retornarUsuarios(String usuario,String senha) throws SQLException{
        getConn();
    ...
    public boolean ehFavorito(String usuario, String anime) throws SQLException{
        String sql = "SELECT 1 FROM favoritos WHERE Usuario ="
                + usuario  + "AND Anime =" + anime ;
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, usuario);
            stmt.setString(2, anime);
            try (ResultSet rs = stmt.executeQuery()) {
                return rs.next(); // Retorna true se achar o registro
            }
        }
